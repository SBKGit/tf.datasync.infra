...
def copy_to_target(final):
    ...
    target_bucket_value_list = [value.strip() for value in target_bucket.split(",")]
   
    success = True  # Initialize a success flag
   
    for value in target_bucket_value_list:
        try:
            print(f"Target Bucket: {value}")
            ...
            print(f'Error copying file to {value}: {str(e)}')
